...
# Cria as abas
tab1, tab2, tab3, tab4,  = st.tabs(["Sobre os Dados", "Análise de Dados", "Modelos de Machine Learning", "Sobre"])



# "Sobre os Dados"
with tab1:
    ## Tela Sobre os Dados - Título
...
with tab2:
    ## Tela de Análise de Dados - Título
...
with tab3:
    ...
    #sns.histplot(df3['SalePrice'],kde=True)
    ...
    #lam = 0.15
    #for col in df3_num :
    #        df3_num[col] = boxcox1p(df3_num[col], lam)  # Transformação Box-Cox aplicada diretamente à coluna
        
        
    #for col in df3_num.columns:
    #    df3_num[col]= np.log1p(df3_num[col])
    ...
    modelo_xgb = XGBRegressor(alpha=0.1, 
                          colsample_bytree=0.5, 
                          reg_lambda=0, 
                          learning_rate=0.05, 
    ...
    # Para MSE
    scores = cross_val_score(modelo_xgb , x_train, y_train, cv=5, scoring='neg_mean_squared_error')
    mean_score_mse = -scores.mean()
    std_score_mse = scores.std()
    
    # Para R²
    scores_r2 = cross_val_score(modelo_xgb, x_train, y_train, cv=5, scoring='r2')
    mean_score_r2 = scores_r2.mean()
    std_score_r2 = scores_r2.std()

    # Para RMSE
    rmse_scores = np.sqrt(-scores)  # Convertendo MSE para RMSE
    mean_score_rmse = rmse_scores.mean()
    std_score_rmse = rmse_scores.std()


    st.write(f'R2: {mean_score_r2}')
    st.write(f'Mean Squared Error: {mean_score_mse}')
    st.write(f'Root Mean Squared Error: {mean_score_rmse}')
...
with tab4:
    st.subheader("Sobre:")
    st.write("O objetivo deste modelo é agregar valor ao meu portfólio, demonstrando meus conhecimentos em análise de dados e \
        desenvolvimento de machine learning. Estou sempre buscando oportunidades para atualizar e aprimorar os códigos à medida \
        que adquiro novas técnicas e insights.\n "
        )

    st.subheader("Melhorias:")

    st.write("Tratamento de Outliers: Inicialmente, tentei remover possíveis outliers aplicando filtros baseados nos valores \
        identificados por meio de boxplots. No entanto, observei uma queda na performance do modelo após essa alteração, e, por \
        isso, decidi, por enquanto, não modificar os outliers. Estou estudando e testando métodos mais precisos e eficazes para lidar \
        com esse problema.\n")

    st.write("Criação de Novas Variáveis: Ao analisar os códigos de outros usuários no Kaggle, percebi que alguns criaram novas colunas de \
        variáveis com base nas já existentes, para melhorar o aprendizado do modelo. Estou estudando essa abordagem e pretendo aplicá-la \
        em futuras versões do modelo.")
